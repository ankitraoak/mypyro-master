...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~mypyrogram.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~mypyrogram.Client.sign_in` or :meth:`~mypyrogram.Client.sign_up`).

        next_type (:obj:`~mypyrogram.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~mypyrogram.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~mypyrogram.Client.resend_code`.
    """
